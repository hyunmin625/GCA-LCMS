...
- [ ] DB 스키마 확장 (courses, sections, lessons, enrollments)
- [ ] 코스 카탈로그 페이지 (`/courses`)
- [ ] 코스 상세 / 소개 페이지 (`/courses/:id`)
- [ ] 수강 등록 플로우 (무료 코스 → 즉시 등록)
- [ ] 강의실 DB 연동 (하드코딩 제거 → Supabase 조회)
- [ ] 마이페이지 (`/dashboard` — 수강 목록, 진행률)
...
- [ ] 관리자 레이아웃 (`/admin/*`)
- [ ] 코스 관리 CRUD (`/admin/courses`)
- [ ] 섹션·레슨 관리 (순서 변경, 드래그)
- [ ] 레슨 편집기 (YouTube URL, 설명, 첨부파일)
- [ ] 콘텐츠 상태 관리 (초안 → 게시)
- [ ] 미디어 에셋 관리 (Supabase Storage — 썸네일, 첨부)
- [ ] 회원 관리 (목록, 역할 변경, 수강 현황)
